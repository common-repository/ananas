...
class Page extends API {
	...
	public function __construct() {
		...
		$shared_link        = ! empty( $settings['shared_link'] ) ? $settings['shared_link'] : "https://www.ananas-analytics.cloud/share/{$domain}?auth=XXXXXXXXXXXX";
		$custom_domain      = ! empty( $settings['custom_domain'] ) ? $settings['custom_domain'] : "analytics.{$domain}";

		$this->fields = [
			'general'     => [
				[
					'label'  => esc_html__( 'Ananasにこのサイトを登録する。', 'plausible-analytics' ),
					'slug'   => 'connect_to_plausible_analytics',
					'type'   => 'group',
					'desc'   => sprintf(
						'%1$s <a href="%2$s" target="_blank">%3$s</a> %4$s',
						esc_html__( 'このサイトのドメインを自動で初期値として設定しました。まだ行っていない場合は、Ananasのダッシュボードにこのサイトの登録を行って下さい。まだAnanasでアカウントを登録していない場合は', 'plausible-analytics' ),
						esc_url( 'https://www.ananas-analytics.cloud/register' ),
						esc_html__( 'こちらからAnanasに登録してください。', 'plausible-analytics' ),
						esc_html__( 'アカウント登録後、このサイトをAnanasダッシュボードに登録してください。', 'plausible-analytics' )
					),
					'toggle' => false,
					'fields' => [
						[
							'label' => esc_html__( 'ドメイン名(Ananasのダッシュボードに登録したドメイン名と同じものを入力)', 'plausible-analytics' ),
							'slug'  => 'domain_name',
							'type'  => 'text',
							'value' => $domain,
						],
				...
				// [
				// 	'label'  => esc_html__( 'Setup custom domain with Ananas Analytics', 'plausible-analytics' ),
				// 	'slug'   => 'is_custom_domain',
				// 	'type'   => 'group',
				// 	'desc'   => sprintf(
				// 		'<ol><li>%1$s <a href="%2$s" target="_blank">%3$s</a></li><li>%4$s %5$s %6$s %7$s %8$s</li></ol>',
				// 		esc_html__( 'Enable the custom domain functionality in your Ananas account.', 'plausible-analytics' ),
				// 		esc_url( 'https://docs.plausible.io/custom-domain/' ),
				// 		esc_html__( 'See how &raquo;', 'plausible-analytics' ),
				// 		esc_html__( 'Enable this setting and configure it to link with Ananas Analytics on your custom domain.', 'plausible-analytics' ),
				...
				// 		esc_html__( 'Create a secure & private shared link in your Ananas account. Make sure the link is not password protected.', 'plausible-analytics' ),
			...
			// 'self-hosted' => [
			// 	[
			// 		'label'  => esc_html__( 'Self-hosted Ananas Analytics?', 'plausible-analytics' ),
			// 		'slug'   => 'is_self_hosted_plausible_analytics',
			// 		'type'   => 'group',
			// 		'desc'   => sprintf(
			// 			'%1$s <a href="%2$s" target="_blank">%3$s</a>',
			// 			esc_html__( 'If you\'re self-hosting Ananas on your own infrastructure, enter the domain name where you installed it to enable the integration with your self-hosted instance. Learn more', 'plausible-analytics' ),
			// 			esc_url( 'https://plausible.io/self-hosted-web-analytics/' ),
			// 			esc_html__( 'about Ananas Self-Hosted.', 'plausible-analytics' )
			// 		),
			// 		'toggle' => true,
			// 		'fields' => [
			// 			[
	...
	public function register_menu() {
		...
		// Setup `Ananas Analytics` page under Settings.
		add_options_page(
			esc_html__( 'Ananas Analytics', 'plausible-analytics' ),
			esc_html__( 'Ananas Analytics', 'plausible-analytics' ),
			'manage_options',
			'plausible_analytics',
			[ $this, 'settings_page' ]
	...
	public function render_page_header() {
		...
		<div class="plausible-analytics-header">
			<div class="plausible-analytics-logo">
				<img src="<?php echo PLAUSIBLE_ANALYTICS_PLUGIN_URL . '/assets/dist/images/icon.png'; ?>" alt="<?php esc_html_e( 'Ananas(アナナス)', 'plausible-analytics' ); ?>" />
			</div>
			<div class="plausible-analytics-header-content">
				<div class="plausible-analytics-title">
					<h1><?php esc_html_e( '設定', 'plausible-analytics' ); ?></h1>
				</div>
	...
	public function statistics_page() {
		...
		if ( $is_shared_link && ! empty( $shared_link ) ) {
			...
			// }
			?>
			<iframe plausible-embed="" src="<?php echo esc_attr("{$shared_link}&embed=true&theme=light&background=transparent"); ?>" scrolling="no" frameborder="0" loading="lazy" style="width: 100%; height: 1750px; "></iframe>
			...
			<div class="plausible-analytics-statistics-not-loaded">
				...
				echo sprintf(
					...
					esc_html( 'to generate your shared link from your Ananas Analytics dashboard. Make sure the link is not password protected.', 'plausible-analytics' ),
					...
					esc_html( 'under Embed Analytics to view Ananas Analytics dashboard within your WordPress site.', 'plausible-analytics' )
				);
				?>
			</p>
			<?php
